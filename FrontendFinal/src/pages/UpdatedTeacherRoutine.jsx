...
import { downloadTeacherDocx } from '../utils/buildTeacherDoc';

const UpdatedTeacherRoutine = () => {
  ...
  // ---------- Mapped schedules for on-screen RoutineTable ----------
  const getMappedSchedules = () => {
    ...
    return teacherDetail.classes.map((cls) => ({
      ...
      sessional: cls.sessional
        ...
        : null,
      teachers: [], // don't show teacher codes on their own routine
    }));
  };
  ...
  const handleDownloadDocx = async () => {
    ...
    try {
      const blob = await downloadTeacherDocx({
        teacherDetail,
        routineMode,
        season: activeSeason,
        year: activeYear,
      ...
      a.href = url;
      a.download = `teacher-routine-${teacherDetail.code || teacherDetail.id}-${activeSeason}-${activeYear}.docx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch (err) {
      console.error('Teacher DOCX generation failed:', err);
    ...
    <div>
      <div className="glass-card no-print" style={{ marginBottom: '2rem' }}>
        ...
        >
          <div
            ...
            style={{ flexGrow: 1, minWidth: '200px', marginBottom: 0 }}
          >
            ...
            >
              ...
              {teachers.map((t) => (
                <option key={t.id} value={t.id}>
                  {t.name} ({t.code || 'N/A'}) - {t.designation}
                </option>
              ))}
            </select>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            <button
              ...
              disabled={!selectedTeacherId || !teacherDetail}
            >
              Download DOCX
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="routine-export-container">
          <UniversityHeader
            subTitle={`Weekly Class Routine — Teacher: ${teacherDetail.name} — ${activeSeason} ${activeYear} ${
          ...
          >
            ...
            <div style={{ textAlign: 'right' }}>
              <strong>Total Assigned Credits:</strong>{' '}
              <span
                style={{ textDecoration: 'underline', fontWeight: 'bold' }}
              >
                {teacherDetail.assignedCredit?.toFixed(1) || '0.0'} Hours
              </span>
            </div>
          </div>
          ...
          >
            ...
            (Level-TermSection) and [Room Number].
          </div>
        </div>
      ) : (
        <div
...
export default UpdatedTeacherRoutine;